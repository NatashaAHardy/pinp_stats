---
title: "PINP seabird detection in long-nosed fur seal diets"
...
#knitr::opts_knit$set(root.dir = "/Users/natasha/Documents/Professional/WORK/CURRENT JOBS/PINP/PINP DATA/pinp_stats")
...
This document contains all code pertinent to:
(i) data manipulation, 
...
These dataframes are currently downloaded as .csv spreadsheets from the diet analysis master document found here: https://docs.google.com/spreadsheets/d/1phfyo3kUyWZfwKF0IAHXgkxy23dbYs_tKkCfEIWeY8g/edit#gid=2115650924 
...
Note that figure 1 was prepared by Harley at PINP using QGIS software. Figure 2 is a composite panel of photos by Natasha Hardy & Rebecca McIntosh.
...
```{r Figure S3a Bird detections / location}
...
```{r Figure S3b Penguin detections / location}
...
```{r Figure S3 ESM Plot Panel - location~time}
...
```{r Figure S1 DNA detections by seabird taxa abundance}
...
```{r Figure SX OTU Proportional Read Abun by OTU}
...
```{r Figure 7 HAPLOTYPES PER SAMPLE!!}
...
```{r Figure 7 extra HAPLOTYPES Bubble Graph}
...
```{r GLM Penguin 0 ~ metric * location}
...
```{r GLM Penguin 1 ~ metric + location}
