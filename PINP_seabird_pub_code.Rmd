...
**- Figure S2a** depicts seabird percentage detections by location & time (using all DNA obtained through standard DNA QC)

```{r Figure S2a - data manip, eval=FALSE}
#Figure 5a
#Colour palette
...
```{r Figure S2a - Bird % detections / location, eval=FALSE}
figS2a_bird <- ggplot(bird_master_group2, aes(x=time_id, y=percentage_detected, fill=metric)) +
...
ggsave(here('figures/ESM/figS2a_bird_pdetect_location.jpeg'), plot = figS2a_bird, width = 8, height = 9, dpi = 300)

```

**- Figure S2b** depicts penguin percentage detections by location & time (using all DNA obtained through standard DNA QC)

```{r Figure S2b Penguin % detections / location, eval = FALSE}
...
#Figure S2b
figS2b_penguin <- ggplot(penguin_master_group2, aes(x=time_id, y=percentage_detected, fill=metric)) +
  geom_bar(stat="identity",position="dodge")+
  coord_flip()+
  ylim(0,75)+
  theme_classic() + 
...
ggsave('figures/ESM/figS2b_penguin_pdetect_location.jpeg', plot = figS2b_penguin, width = 8, height = 9, dpi = 300)


```

**- Figure S2** Includes both pannels for a) seabird and b) penguin percentage detections at sampling sites and seasons.

```{r Figure S2 PANEL - location~time, eval = FALSE}
...
#combine using cowplot
bird_panel_ms = plot_grid(figS2a_bird, figS2b_penguin, 
                           ncol = 2
                          #labels = c("a) Seabirds", "b) Little penguins (Eudyptula minor)"), 
                          #label_size = 18, ncol = 2, 
                          #label_x = 0.20, label_y = 1, hjust = 0, vjust = 1.5,
...
ggsave(here("figures/ESM/figS2_panel_ms.pdf"), plot = bird_panel_ms, width = 12, height = 10, scale = 0.95, dpi = 300)

ggsave(here("figures/ESM/figS2_panel_ms.jpeg"), plot = bird_panel_ms, width = 12, height = 10, scale = 0.95, dpi = 300)
...
**- OTU pallettes** created to associated each taxon with a specific colour scheme throughout the OTU graphs.
...
```{r OTU Alternative colour palette}
...
**- Figure 5** to illustrate the proportional abundance of sequences per taxon per sample, also known as the relative rea abundance (Thomas & Deagle, et al. 2019). For ESM.

```{r Figure 5 OTU Relative Read Abun by Total per Sample, eval = FALSE}
...
**- Figure 6** Little penguin haplotype diversity within samples containing DNA for little penguin (n = 10), using the primer for 12S rRNA gene. The genetic abundance of haplotypes identified is overlaid. Each haplotype within a sample represents an individual penguin consumed. 

```{r Figure 6 HAPLOTYPES PER SAMPLE, eval=FALSE}
...
```{r Figure 6 or S4 extra HAPLOTYPES Bubble Graph, eval=FALSE}
